...
def verify_dnie_thread_func(pin):
    ...
    try:
        ini = md.DataManager(pin) # Aqui se inicializa la instancia de manejo de datos y el resto de programas trabajaran sobre esa instancia. 
...
def iniciar_verificacion():
    ...
    # Configuracion de las Fuentes dinámicas
    title_font = pygame.font.Font(None, font_sizes["title"])
    subtitle_font = pygame.font.Font(None, font_sizes["normal"])
    input_font = pygame.font.Font(None, font_sizes["subtitle"])
    status_font = pygame.font.Font(None, font_sizes["normal"])
    ...
    class Button:
        ...
        def draw(self, surface):
            ...
            text_surf = subtitle_font.render(self.text, True, config.get_color("text_primary"))
    ...
    title_surf = title_font.render("Verificación del DNIe", True, config.get_color("text_primary"))
    title_rect = title_surf.get_rect(center=(WIDTH // 2, 50))
    subtitle_surf = subtitle_font.render("Por favor, introduce el PIN de tu DNIe:", True, config.get_color("text_primary"))
    subtitle_rect = subtitle_surf.get_rect(center=(WIDTH // 2, 100))
    input_rect = pygame.Rect(WIDTH // 2 - 150, 130, 300, 40)
    pin_text = ""
    input_active = False
    verify_button = Button(WIDTH // 2 - 75, 190, 150, 40, 'Verificar', config.get_color("success"), config.get_color("success_hover"))
    ...
    while running:
        ...
        if is_verifying:
            ...
            status_surf = status_font.render("Verificando...", True, config.get_color("text_primary"))
            ...
            border_color = config.get_color("accent_primary") if input_active else config.get_color("border")
            pygame.draw.rect(screen, border_color, input_rect, 2, 5)
            asterisks_surf = input_font.render("*" * len(pin_text), True, config.get_color("text_primary"))
